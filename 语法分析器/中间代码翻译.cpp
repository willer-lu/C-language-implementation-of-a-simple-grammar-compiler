#include"标头.h"
enum dif{ I, C, K, P, Ch, St, default };//TOKEN的种类
char t[100][15];//临时变量
enum type{number,ch,id,undefine};//对象的种类
char actid[20][10] = { "+","-","*","/",">","<",">=","<=","==","=","if","else","ie","while","do","we" ,"return","cout","cin","#"};//动作
int sem[1000];//语义栈
...
int act(char acti[20][10],char *cmp) {//寻找动作序号
	for (int i = 0;i<20;i++){
		if (strcmp(acti[i], cmp) == 0) return i+1;
	}
	return 0;
}
void actanalysis() {//动作分析 
	int i = 0;
	int j = 0;
	while (1) {
		if (strcmp(sport[i].name, "#") == 0)break;
		if (act(actid, sport[i].name))//找到对应动作
	...
	for (int i = j; i >= 0; i--) {//完成四元式，采取自底向上翻译文法
		...
		if ((token[i].kind == I) || (token[i].kind == C) || (token[i].kind == Ch)) {//为标识符 常数 字符的情况
			sem[loca++] = i;
			continue;
		}
		if ((token[i].kind == P) && (token[i].value2 <= 10)) {//不同情况界符 对应四元式生成
...
void range(char c[30]) {//改进输出格式
	int i = strlen(c);
	while (i++ < 16)cout << " ";
	return;
}
void printact() {//打印 以及输出四元式
	cout << "四元式如下:" << endl;
	for (int i = 0; i < 100; i++) {
	...
	ofstream act3;
	act3.open("fourelem.txt", ios::out);//具体的四元式
	int he = 0;
	for (int i = 0; i < 100; i++) {
	...
	act3.close();
	ofstream act4;
	act4.open("fourelement.txt", ios::out);//按四元式结构储存 便于后续使用
	for (int i = 0; i < 100; i++) {
...
void translate1() {
	ifstream act1,act2;
	act1.open("action.txt", ios::in);//动作序列
	act2.open("wordsequence.txt", ios::in);//单词序列
